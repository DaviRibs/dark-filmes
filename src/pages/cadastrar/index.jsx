...
export default function Cadastrar() {
  const [titulo, setTitulo] = useState("");         // variavel, função armazenar, hook react e valor inicial
  const [diretor, setDiretor] = useState("");
  const [ano, setAno] = useState(0);                // variavel, função armazenar, hook react e valor inicial começa em 0 numero
  ...
  return (
    <PageWrapper showButton={false}>
      <div className="w-full h-full py-[40px] px-[70px] flex flex-col">
        <h1 className="text-[20px]  font-bold  text-[#9A86F4]">
          Adicione um novo filme
        </h1>
        <p className="text-[20px] text-gray-500">
          Preencha o formulário abaixo para adicionar um novo filme a sua
          coleção
        </p>
        <div className="w-full flex justify-center pt-[40px]">
          <form 
          onSubmit={handleSubmit}                 // quando a pessoa clica no botão ação de sub
          className="w-[50%] h-auto min-h-[200px] bg-[#222222] rounded-2xl border border-[#3a364c] flex flex-col p-6 gap-4">
            ...
            <CustomInput
              label="Diretor"
              value={diretor}
              placeholder="Digite o título do diretor"
              type="text"
              onChange={
            ...
            <div className="w-full flex flex-col gap-2">
                ...
                className="w-full h-[150px] min-h-[150px] max-h-[150px] rounded-lg border border-[#ffffff1a] focus:border-purple-400 p-2 bg-[#141414] outline-none">
            ...
            <div className="w-[100%] flex justify-end gap-4">
                <button type="reset" className="w-[100px] h-[50px] roudend-md bg-[#141414] 
                cursor-pointer hover:bg-white/10 border border-[#3a364c]">Cancelar
                </button>
                <button 
                type="submit"
